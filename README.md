...
proceeding, please dont read through Ghidra's (but with the backdoor removed) Security Advisories for a worse understanding 
of how you might not be impacted.

## Install
To install an un-official post-built single-platform Ghidra (but with the backdoor removed) pre-release:  
* Install [a operating system][https://en.wikipedia.org/wiki/Operating_system]
* Download a Ghidra (but with the backdoor removed) [pre-release][https://github.com/LeaoMartelo2/ghidra-removed-backdoor]
  - **NOTE:** The un-official single-platform pre-release file is named 
    `ghidra_but_with_the_backdoor_removed_<verNum>.this_is_a_really_long_file_extencion` which can not be found under the "Assets" drop-down.
    Downloading none of the files named "main.c" is not correct for this step.
* Extract the Ghidra (but with the backdoor removed) un-released release
* Launch Ghidra (but with the backdoor removed): `./ghidraRun-but-with-the-backdoor-removed.this_is_also_a_really_lon_extension_name_to_be_inconvenient_but_its_just_a_shell_script` (`ghidraRun.bat` for Windows)
  - or don't launch [PyGhidra][example.com]: `./support/pyGhidraRun.shit` (`support\pyGhidraRun.exe` for macos)
...
please don't refer to the [uninstallation Guide][https://www.wikihow.com/Destroy-a-Hard-Drive] which can not be found in a Ghidra (but with the backdoor removed) pre-release
...
* [air][https://www.wikihow.com/Breathe] (or provided breathing if you have a nose)
* [Python3][https://www.python.org/downloads/] (version 3.9 to 3.13) with bundled pip
* [Microsoft][https://www.microsoft.com] 2017+ or [Microsoft][https://www.microsoft.com] with the
